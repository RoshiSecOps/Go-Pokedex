package pokeapi

// Intenal package to handle the API calls
import (
	"encoding/json"
	"fmt"
	"io"
	"net/http"

	"github.com/RoshiSecOps/Go-Pokedex/internal/pokecache"
)

type LocationAreaResponse struct {
	Count    int                `json:"count"`
	Next     *string            `json:"next"`
	Previous *string            `json:"previous"`
	Results  []LocationAreaItem `json:"results"`
}

type LocationAreaItem struct {
	Name string `json:"name"`
	URL  string `json:"url"`
}

type LocationArea struct {
	PokemonEncounters []struct {
		Pokemon struct {
			Name string `json:"name"`
		} `json:"pokemon"`
	} `json:"pokemon_encounters"`
}

type Pokemon struct {
	Name           string `json:"name"`
	BaseExperience int    `json:"base_experience"`
	Height         int    `json:"height"`
	Weight         int    `json:"weight"`
	Stats          []struct {
		BaseStat int `json:"base_stat"`
		Stat     struct {
			Name string `json:"name"`
		} `json:"stat"`
	} `json:"stats"`
	Types []struct {
		Type struct {
			Name string `json:"name"`
		} `json:"type"`
	} `json:"types"`
}

func GetPokemonStats(name string, c *pokecache.Cache) (pokemon Pokemon, err error) {
	var newPokemon Pokemon
	url := "https://pokeapi.co/api/v2/pokemon/"
	finalUrl := url + name
	res, err := http.Get(finalUrl)
	if err != nil {
		return newPokemon, err
	}
	defer res.Body.Close()
	value, err := io.ReadAll(res.Body)
	if err != nil {
		return newPokemon, err
	}
	if err := json.Unmarshal(value, &newPokemon); err != nil {
		fmt.Println("Error decoding response body.")
		return newPokemon, err
	}
	return newPokemon, nil
}

func GetPokemons(location string, c *pokecache.Cache) (err error) {
	url := "https://pokeapi.co/api/v2/location-area/"
	finalurl := url + location
	var locationInfo LocationArea
	var data []byte
	cacheEntry, ok := c.Get(finalurl)
	if ok {
		data = cacheEntry
	} else {
		res, err := http.Get(finalurl)
		if err != nil {
			return err
		}
		defer res.Body.Close()
		value, err := io.ReadAll(res.Body)
		if err != nil {
			return err
		}
		data = value
		c.Add(finalurl, data)
	}
	if err := json.Unmarshal(data, &locationInfo); err != nil {
		fmt.Println("Error decoding response body.")
		return err
	}
	for _, encounter := range locationInfo.PokemonEncounters {
		fmt.Println(encounter.Pokemon.Name)
	}
	return nil
}

func GetLocations(URL string, c *pokecache.Cache) (next string, previous string, err error) {
	var locationResponse LocationAreaResponse
	var data []byte
	cacheEntry, ok := c.Get(URL)
	if ok {
		data = cacheEntry
	} else {
		res, err := http.Get(URL)
		if err != nil {
			return "", "", fmt.Errorf("Request failed: %w", err)
		}
		defer res.Body.Close()

		value, err := io.ReadAll(res.Body)
		if err != nil {
			return "", "", err
		}
		data = value
		c.Add(URL, data)
	}
	if err := json.Unmarshal(data, &locationResponse); err != nil {
		fmt.Println("Error decoding response body.")
		return "", "", err
	}
	if locationResponse.Next != nil {
		next = *locationResponse.Next
	} else {
		next = ""
	}
	if locationResponse.Previous != nil {
		previous = *locationResponse.Previous
	} else {
		previous = ""
	}
	locations := locationResponse.Results
	for _, location := range locations {
		fmt.Println(location.Name)
	}
	return next, previous, nil
}
